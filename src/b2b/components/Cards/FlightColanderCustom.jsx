      ...
      try {
        ...
        for (let i = 1; i <= lastDateofMonth; i++) {
          if (
            ...
            for (let k = 0; k < response?.data?.dates?.length; k++) {
              if (
                response?.data?.dates[k] ===
                `${currYear}-${
                ...
                availArray.push(response?.data?.dates[k]);
              }
            }
        ...
        <div className="calendar">
          ...
          <ul className="grid grid-cols-7 gap-5 place-items-center">
            {currDays?.map((item, index) => (
              ...
              <li
                className={`${
                  item?.active ? " text-gray-300 " : " text-gray-100 "
                } ${
                  item?.isToday
...
export default FlightColanderCustom
