...
function StarCategorySubAgentMarkupModal({
  setIsModalOpen,
  setStarMarkupData,
  starMarkupData,
  name,
  ...
  const [markupType, setMarkupType] = useState(
    starMarkupData?.subAgentMarkup?.markupType || ""
  );
  ...
  const submitHandler = async (e) => {
    try {
      ...
      const response = await axios.patch(
        `/b2b/hotels/markup/sub-agent/star-category/upsert`,
        {
          markup: markupAmount,
      ...
      setStarMarkupData((prev) => {
        return { ...prev, subAgentMarkup: response.data };
      });
      setIsModalOpen({ client: false, subAgent: false });
      ...
      <div className="fixed inset-0 w-full h-full temp_lightglass flex items-center justify-center z-20 ">
        ...
        >
          <div className="flex items-center justify-between border-b p-4">
            <h2 className="font-medium mb-2">Sub Agent Markup</h2>
            <button
...
export default StarCategorySubAgentMarkupModal;
