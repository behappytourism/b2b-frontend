...
function AgentMarkupModal({
  ...
  useHandleClickOutside(wrapperRef, () =>
    setIsMarkup({ client: false, agent: false })
  );
  ...
  const [markupType, setMarkupType] = useState(
    markupData?.subAgent?.markupType || ""
  );
  ...
  const submitHandler = async (e) => {
    try {
      ...
      const response = await axios.patch(
        `/b2b/hotels/markup/sub-agent/room-type/upsert`,
        {
          markup: markupAmount,
      ...
      setIsLoading(false);
      setMarkupData((prev) => {
        return { ...prev, subAgent: response.data };
      });
      setIsMarkup({ client: false, agent: false });
        ...
        >
          <div className="flex items-center justify-between border-b p-4">
            ...
            <button
              ...
              onClick={() => setIsMarkup({ client: false, agent: false })}
            >
              <MdClose />
            </button>
          </div>
...
export default AgentMarkupModal;
