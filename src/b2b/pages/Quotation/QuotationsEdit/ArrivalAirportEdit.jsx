...
import { setArrivalAirportDisabled, clearArrivalEditPageOnSideClearButton } from '../../../../redux/slices/quotationSlice';
import { setArrivalAirport } from '../../../../redux/slices/quotationSlice';
import { AiOutlineClose } from "react-icons/ai";
import lottieArrival from '../../../../static/B2bLotti/animation_ln8ocu6t.json'
import Lottie from "lottie-react";

function ArrivalAirportEdit({setQuotationEdit, isLoading}) {
  const { airports, arrivalAirport } = useSelector((state) => state.quotation);


    const dispatch = useDispatch()
    const [ search, setSearch ] = useState(arrivalAirport?.name)
    const [ filteredAirport, setFilteredAirport ] = useState([])
    
    useEffect(()=>{
        setSearch(arrivalAirport?.name)
    },[arrivalAirport?.name])
    ...
    <div className="max-w-screen-sm mx-auto">
      <div className="p-4 ml-4 lg:ml-0 lg:p-0">
        ...
        <div className="mt-2 relative flex lg:gap-10 gap-2 md:gap-5">
          ...
          <div className='flex'>
        <div className="mt-5 space-y-2 max-h-[80vh] ">
...
export default ArrivalAirportEdit
