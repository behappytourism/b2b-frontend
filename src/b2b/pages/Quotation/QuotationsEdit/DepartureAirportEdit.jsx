...
function DepartureAirportEdit({ setQuotationEdit }) {


  const { airports, departureAirport } = useSelector((state) => state.quotation);
    const dispatch = useDispatch();
    const [search, setSearch] = useState(departureAirport?.name);
    ...
    return (
      <div className="max-w-screen-sm mx-auto">
        <div className={`${isEffect ? " scale-0 " : " scale-100 "} transition-all duration-500 p-4 ml-4 lg:ml-0 lg:p-0`}>
          <div className="pb-1">
          ...
          <div className="mt-5 lg:space-y-2 space-y-1  max-h-[80vh]">
            {filteredAirport?.map((airport, index) => (
              ...
              >
                <div className="font-[600]"> {airport?.airportName} - {airport?.terminals.terminalCode}</div>
              </div>
            ))}
          </div>
          <div className="pt-10 ">
...
export default DepartureAirportEdit
