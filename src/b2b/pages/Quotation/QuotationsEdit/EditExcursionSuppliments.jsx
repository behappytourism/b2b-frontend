...
import EditExcSupplimentCards from "./EditExcSupplimentCards";
import Carousel from "react-multi-carousel";
import "react-multi-carousel/lib/styles.css";
import SupplimentEditsModal from "./SupplimentEditsModal";
import EditCarouselSupBtn from "./EditCarouselSupBtn";
...
function EditExcursionSuppliments({ setQuotationEdit }) {
  ...
  const fetchExcursions = async () => {
    try {
      ...
      dispatch(setInitialEditExcursionSuppliment(res.data))
      setIsLoading(false)
    } catch (error) {
      setIsLoading(false)
      console.log(error); 
  ...
  useEffect(()=>{
    if(!excursionSuppFilteredNew?.length) {
      fetchExcursions()
    }
  }, [])
  ...
  const selectedExcursions = excSupplimentFiltered?.filter(
    (exc) => exc.isSelected
  );
  ...
  useEffect(() => {
    if (totalDays < selectedExcursions?.length) {
      setShowWarningModal(true);
    }
  }, [totalDays < selectedExcursions?.length]);

  const handleRomveMoreThanDay = () => {
    dispatch(
      removeMorthanOneInDateSuppliment(selectedExcursions[lastPushedData])
    );
  };
        ...
        {/* {selectedExcSupplimentsIds?.length > 0 ? (
          <div className=" md:rounded-lg bg-black grid md:flex md:first-letter md:w-2/3 md:p-4 gap-6 md:ml-32 mb-20 md:mb-0 p-2 ">
            <div className="md:ml-5 ml-36">
              <img
                src={
                  selectedExcSupplimentsIds[lastPushedData]?.image?.length > 0
                    ? config.SERVER_URL +
                    selectedExcSupplimentsIds[lastPushedData]?.image[0]
                    : previewImage
                }
                alt="img"
                  ...
                  <div className=" rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                    ...
                    <div className="relative p-6 flex-auto">
                      <p className="my-4 text-slate-600 text-lg leading-relaxed">
                        You are exceeded the average number of excursion can be
                        visited per day.
                      </p>
                    </div>
                    {/*footer*/}
...
export default EditExcursionSuppliments;
