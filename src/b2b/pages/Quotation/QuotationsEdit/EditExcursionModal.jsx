...
import EditSideExcTransferModal from "./EditSideExcTransferModal";




function EditExcursionModal({ data, select, set }) {
  const { setModal } = data;
  const dispatch = useDispatch();
  const { selectedExcursionIds } =
    useSelector((state) => state.quotation);

  const handleRemove = (item) => {
    const removedData = selectedExcursionIds.filter((ele) => ele !== item);
    set(removedData);
    dispatch(addExcursion(removedData));
    dispatch(handleEditRemoveTransfer(item));
  };

  const selectedExcursions = selectedExcursionIds.filter((exc) => exc.isSelected);
    ...
    <div className="">
      <div>
        <div
          id="defaultModal"
          aria-hidden="true"
          className="fixed hidden  md:flex justify-center items-center backdrop-blur-sm top-0 md:bottom-0 md:top-0 md:left-0 md:right-0 z-50 w-full md:p-4 md:overflow-x-hidden overflow-y-auto"
        >
          <div className="relative shadow-lg max-w-1xl max-h-full lg:w-[1000px] w-full">
            <div className="lg:relative bg-white lg:rounded-lg shadow-lg dark:bg-white p-5 h-[850px] md:h-auto  transition-all duration-300 ">
              ...
              <div className="grid  md:grid-cols-5 gap-0 md:border-t-2">
                {selectedExcursionIds?.map((ele, index) => {
                  return (
                    <>
                      ...
                      <div className=" md:pt-3 max-w-[100px] md:ml-10">
                        {ele?.selectedExcursionType !== "TicketWithTransfer" ? (
                          <h2 className=" bg-green-100 p-2 text-gray-300 text-[10px] ">
                            Transfer :{" "}
                            <span className="text-green-500 ">
                              {" "}
                              {ele?.selectedExcursionType}
                            </span>{" "}
                          </h2>
                        ) : (
                          ""
              ...
              <div className="flex justify-end pt-4">
                <button
                  onClick={() => setModal(false)}
                  className="bg-blue-400 text-white rounded text-xs p-1 w-14 mr-3"
                >
                  Done
                </button>
              </div>
                ...
                {selectedExcursionIds?.map((ele, index) => {
                  return (
                    ...
                    <div className="flex">
                      ...
                      <div className="">
                        <h3 className="text-xs md:pt-5 font-medium text-[#495057] md:max-w-[300px] text-center">
                          {ele?.attraction}
                        </h3>
                      </div>
                      <div className="md:w-60">
                      ...
                      <div className=" md:pt-3 max-w-[100px] md:ml-10">
                        {ele?.selectedExcursionType !== "TicketWithTransfer" ? (
                          <h2 className=" bg-green-100 p-2  text-gray-300 text-[10px] max-w-sm hidden md:block">
                            Transfer :{" "}
                            <span className="text-green-500 ">
                              {" "}
                              {ele?.selectedExcursionType}
