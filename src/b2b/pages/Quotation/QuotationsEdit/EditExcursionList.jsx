...
import EditExcCards from "./EditExcCards";
import Carousel from "react-multi-carousel";
import "react-multi-carousel/lib/styles.css";
import EditExcursionModal from "./EditExcursionModal";
import EditCarouselExcBtn from "./EditCarouselExcBtn";
...
function EditExcursionList({ setQuotationEdit }) {
  ...
  const { selectedExcursion, excursionFiltered, checkInDate, checkOutDate, excursionFilteredNew, searcheExcursions, selectedExcursionIds } =
  ...
  const fetchExcursions = async () => {
    try {
      ...
      dispatch(setInitialEditExcursion(res.data))
      setIsLoading(false)
    } catch (error) {
      setIsLoading(false)
      console.log(error);
  ...
  useEffect(()=>{
    if(!excursionFilteredNew.length) {
      fetchExcursions()
    }
  }, [])
      ...
      <div classsName="">
        ...
        <div className=" md:flex md:gap-64 md:justify-between">
          <div>
            ...
            <div className="mt-2 relative p-2 md:p-0 lg:mb-3 w-[423px]">
              <input
          ...
          {selectedExcursionIds.length > 0 ? (
            ...
            <div className="md:mt-2 ml-28 md:ml-0">
              <button
                onClick={() => {
                  dispatch(clearDataDisabled(false));
                  setQuotationEdit({
        ...
        <div className="pt-5 p-1 md:p-0">
          ...
          //   customButtonGroup={<EditCarouselExcBtn />}
          // >
          ...
          <div className="md:rounded-lg bg-black grid md:flex md:first-letter md:w-2/3 md:p-4 gap-6 md:ml-32 mb-20 md:mb-0 lg:p-2">
              ...
              <img
                src={
                  selectedExcursionIds[lastPushedData]?.image?.length > 0
                    ? config.SERVER_URL +
                    selectedExcursionIds[lastPushedData]?.image[0]
                    : previewImage
                }
                alt="img"
            ...
            <div className="md:mt-4 ml-16 md:ml-0">
              <h1 className="text-white text-xs">
                {selectedExcursionIds[lastPushedData]?.attraction}! Great choice,
                keep adding
              </h1>
            </div>
            <div className="justify-end items-end mt-2 ml-36 md:ml-0">
...
export default EditExcursionList;
