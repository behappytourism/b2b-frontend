...
import { addExcursion , handleEditRemoveTransfer, removeIdsExc } from '../../../../redux/slices/quotationSlice'
import TransferType from "./TransferType";
import { previewImage } from "../../../../static/imagesB2B";
import { config } from '../../../../constants';
import ExcTransferModal from './ExcTransferModal';



function ExcursionModal({data, select, set}) {
    const { setModal } = data 
    const dispatch = useDispatch()
    const { selectedExcursionIds} = useSelector((state)=> state.quotation)
                    ...
                    { selectedExcursionIds?.map((ele, index)=> {
                        return (
                         ...
                         <img
                            ...
                            <ExcTransferModal data={ele?._id} tp={ele} index={index} />
                            </div>
                    ...
                    { selectedExcursionIds?.map((ele, index)=> {
                        return (
                         ...
                         <img
                            ...
                            <TransferType data={ele?._id} tp={ele} index={index} />
                            </div>
                                <div className=' md:pt-3 max-w-[100px] md:ml-20' >
                                {
                                    ele?.selectedExcursionType !== "TicketWithTransfer" ? (
                                        <h2  className=' bg-green-100 p-2  text-gray-300 text-[10px] max-w-sm hidden md:block'>Transfer : <span className='text-green-500 '>  {ele?.selectedExcursionType}</span>  </h2>
                                    ) : ""
