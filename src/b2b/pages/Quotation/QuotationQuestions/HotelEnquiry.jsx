...
import { setIsHotelQuotationDisabled, setTransferQuotationDisabled, setAlreadyBooked, setPickPlaceForWithoutHotel, clearPickUpLocation } from "../../../../redux/slices/quotationSlice";
import { AiFillWarning } from "react-icons/ai";
import { config } from "../../../../constants";
import GoogleAutoCompletePlaces from "./GoogleAutoCompletePlaces";


function HotelEnquiry({ setQuestions, setSteps }) {
  ...
  return (
    <div className="max-w-screen-lg mx-auto space-y-10 ">
        ...
        <div className="grid lg:grid-cols-3 md:flex md:w-[600px] gap-3 lg:ml-44  md:ml-20 ml-10 ">
          ...
          onClick={() => {
           ...
           className="relative  cursor-pointer  bg-blue-500 to-sky-100   hover:-translate-y-1 transition-all duration-400 rounded-md border-[1px] border-gray-100 border-opacity-20 shadow-sm w-80 lg:w-[400px] py-3  flex flex-col items-center">
            <div className="">
              <p className="text-sm font-[400] text-white">I have Already Booked</p>
            </div>
          </div>
          <div
            onClick={() => {
      ...
      {showModal ? (
        <>
          <div
            className="justify-center items-center flex  fixed md:inset-0 z-50 outline-none focus:outline-none backdrop-blur-sm transition-all "
          >
            <div className="relative w-auto my-6 mx-auto max-w-2xl">
              {/*content*/}
              <div className=" rounded-lg shadow-lg relative flex flex-col w-80 md:w-full  bg-white outline-none focus:outline-none">
                {/*header*/}
      ...
      {stays?.map((stay, stayIndex) => {
        return (
          <>
          {
            ...
            <div>
              <h1 className='text-center font-bold text-lg'>Selected Hotels</h1>
            </div>
            ...
            <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-3 ">
              {stay.hotels?.map((hotel, hotelIndex) => (
                <div
                  key={hotelIndex}
                  className="relative cursor-pointer bg-white hover:-translate-y-1 transition-all duration-300 rounded-md border-[1px] border-gray-100 border-opacity-20 shadow-round w-full  flex flex-col items-center "
                >
                  ...
                  <div className="p-2 text-center">
                    ...
                    <div className="grid-row-1 p-1">
                      ...
                      <div className=" text-left">
                        <p className="text-[11px]  text-gray-400 ">
                          Board type : {hotel?.boardTypeName ? hotel?.boardTypeName : hotel?.boardTypeCode}
                        </p>
                      </div>
                      </div>
                    <div className="p-2 text-left">
                      <div className="text-xs text-grayColor">
                        {hotel?.hotelData?.address ? hotel?.hotelData?.address : hotel?.city}
                      </div>
                      <div className="text-xs text-grayColor">
                        {hotel?.placeName +
                          " , " +
                          hotel?.hotelData?.state?.stateName ? hotel?.hotelData?.state?.stateName : hotel?.country}
                      </div>
                    </div>
                  </div>
                </div>
