...
function SupplimentsTransferType({data, tp, index}) {
    ...
    const { noOfAdults, noOfChildren, checkInDate } = useSelector((state) => state.quotation);
    ...
    const changeType = (e, id, nm) =>{
      ...
      if (value === "Private") {
        ...
        if (existData) {
          ...
          dispatch(handleSelectedExcursionSupplimets(selectedData));
        } else {
          setSelectData((prevState) => ({
            ...prevState,
            [nm]: value,
        ...
        if (existData) {
          ...
          dispatch(handlechangeEmptyVehicleTypeSuppliments(selectedData));
          dispatch(
            setFilteredSuppliments({
              id: id,
              name: "vehicleType",
          ...
          dispatch(handleSelectedExcursionSupplimets(sltData));
        }
    ...
    const changeSeate = (e, id)=>{
        ...
        if(selectData.value === "Private") {
          ...
          dispatch(setVehiclTypeForEcxSuppliment(datas))
        }
          };
          ...
          useEffect(()=>{
            vehicleCount?.map((ele)=>{
              if(ele?.count){
                  dispatch(vehicleInitialCountSuppliment({count: ele?.count, vehicle: ele?.vehicle?._id, excursionId: data})) 
              }
            })
        ...
        <div className="w-full flex flex-wrap justify-center items-center gap-2 text-xs text-grayColor p-3">
          <div className="flex">
            {tp?.ticketOnlyPricing === true ? (
               <button className={`border-y border-l hover:border-blue-300 text-gray-800 font-bold py-1 px-2 md:py-2 md:px-4 text-[6px] md:text-[9px]  ${
          ...
          <div className="">
           ...
           {tp?.ticketSharedPricing === true ? (
            <>
              ...
              />
              <label htmlFor="" className="text-[14px] text-black">Shared</label>
            </>
           ):""}
            {tp?.ticketVehicleType?.length > 0 ? (
              ...
              />
              <label htmlFor="" className="text-[14px] text-black">Private</label>
              </>
            ):""}
                 ...
                 {selectType === "Private" && selectionColor !== "Ticket" && (
                  <div className="grid md:grid-cols-1 justify-start bg-white w-full h-280 p-3 ml-5 animate-pulse">
                   ...
                   <div className="mt-2 h-3 w-20 bg-gray-100 rounded animate-pulse"></div>
                      {/* {tp?.ticketVehicleType?.map((vcl, i) => {
                        return (
                          <div key={i} className="">
                            <div className=" flex gap-6 p-1 animate-pulse">
                              <label htmlFor="" className="text-black animate-pulse">
                                {vcl?.vehicle?.name}
                              </label>
                              <div className="">
                                <select
...
export default SupplimentsTransferType
