...
function ShowAttractionDetails({ele, orderAttractionDetails, orderDetails}) {
   ...
   const handleDownloadTicket = async (item) => {
    try {
        const res = await axios.get( `/b2b/attractions/orders/${orderAttractionDetails?._id}/ticket/${ele?._id}/single/${item?.ticketNo}`,
        {
            headers: { Authorization: `Bearer ${token}`},
            responseType: "arraybuffer"
        }
    ...
    <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody className=''>
            ...
            >
                ...
                <td className="px-6 py-4 text-xs">
                  <h1 className='text-sm max-w-sm'>{ele?.activity?.name}</h1>
                    <div className='flex gap-1'>
                        <h1 className='w-14 h-4 bg-blue-500 text-white text-center'>Adult {ele?.adultsCount ? ele?.adultsCount : 0}</h1>
                        <h1 className='w-14 h-4 bg-blue-500 text-white text-center'>Child {ele?.childrenCount ? ele?.childrenCount : 0}</h1>
                        <h1 className='w-14 h-4 bg-blue-500 text-white text-center'>Infant {ele?.infantCount ? ele?.infantCount : 0}</h1>
                    </div>
                </td>
                ...
                <td className="px-6 py-4">
                    <h1 className={`${ele?.status === 'confirmed' ? "bg-green-100 text-green-600" : ""} ${ele?.status === 'booked' ? "bg-orange-100 text-orange-500" : ""} font-bold  w-20 text-center p-1 text-xs  rounded`}>
                       {ele?.status}
                    </h1>
                </td>
                <td className="px-6 py-4">
    ...
    </table>
    {
        showDetails && (
            ...
            <div className='flex justify-between'>
                ...
                <div>
                     <h1 className='text-sm max-w-sm font-semibold'>{ele?.activity?.name}</h1>
                     <div className='pt-2'>
                       <h1>{ele?.activity?.attraction?.title}</h1>
                     </div>
                     <div className='pt-1'>
                    <h1 className='text-xs text-gray-300'>    {ele?.date?.slice(0, 10)}</h1>
                </div>
                   </div>
                </div>
                ...
                <div>
                    ...
                    <div className='pt-1'>
                        ...
                        <div className='flex gap-1 pt-1 '>
                            ...
                            <h1  className={`${ele?.status === 'confirmed' ? "bg-green-100 text-green-600" : ""} ${ele?.status === 'booked' ? "bg-orange-100 text-orange-500" : ""} font-semibold  w-20 h-4 text-center  text-xs  rounded`}>{ele?.status}</h1>
                        </div>
                        <div className='pt-1'>
                            <h1 className='text-sm'>Adult Count : {ele?.adultsCount}</h1>
                        </div>
                        <div className='pt-1'>
                            <h1 className='text-sm'>Children Count : {ele?.childrenCount}</h1>
                        </div>
                        <div className='pt-1'>
                            ...
                            ele?.childTickets?.map((item, i)=> {
                                return (
                                    <div className='flex gap-1' key={i}>
                                        ...
                                        className='text-blue-400 cursor-pointer'><IoDownloadOutline /></h1>
                                    </div>
                                )
                            })
                        }
