...
function AgentVisaMarkupModal({ setMarkup, setMarkupData, markupData, visa }) {
  console.log(visa);
  const { id } = useParams();
  ...
  const submitHandler = async (e) => {
    try {
      ...
      const response = await axios.patch(
        "/b2b/subagent/visa/markup/upsert",
        {
          markup: markupAmount,
      ...
      <div className="fixed inset-0 w-full h-full lightglass flex items-center justify-center z-20 ">
        ...
        >
          <div className="flex items-center justify-between border-b p-4">
            <h2 className="font-medium mb-2">Visa Agent Markup</h2>
            <button
